...
public class NetAudioPagerAdapter extends BaseAdapter {
    ...
    private List<NetAudioPagerData.ListBean> mDatas;
        ...
        switch (itemViewType) {
            case TYPE_VIDEO://视频
                ...
                if (top_comments != null && top_comments.size() > 0) {
                    String content = top_comments.get(0).getContent();
...
//                    x.image().bind(viewHolder.iv_image_icon, mediaItem.getImage().getBig().get(0));
                    Glide.with(context).load(mediaItem.getImage().getBig().get(0)).apply(new RequestOptions().placeholder(R.drawable.bg_item).error(R.drawable.bg_item)).into(viewHolder.iv_image_icon);
                }
                break;
            case TYPE_TEXT://文字
                bindData(viewHolder, mediaItem);
            ...
            case TYPE_GIF://gif
                ...
                Glide.with(context).load(mediaItem.getGif().getImages().get(0)).into(viewHolder.iv_image_gif);
    ...
    private View initView(View convertView, int itemViewType, ViewHolder viewHolder) {
        switch (itemViewType) {
            case TYPE_VIDEO://视频
                convertView = inflater.inflate(R.layout.all_video_item, null, false);
                //在这里实例化特有的
